...
- 在同一 MCP 会话中完成工具发现、主题版本、组件目录、目标页面和代表性 Pattern raw content 检查。
- 有效组件目录存在时只抽查一个结构最复杂的业务 Pattern 和一个公共同步 Pattern，不逐个打开后台编辑器。
- Theme Options 只检查本次会修改的标签；纯页面内容搭建不做 Theme Options 全量浏览。
- 未知主题 Pattern 无法采集时，允许改用原生 Gutenberg Blocks 创建 Draft；不得手写近似主题 Pattern，也不得因此发布。
...
只有 3.1 失败、没有有效组件目录，且当前页面确实需要主题专有 Pattern 时，才为该主题版本建立 Draft/Private 的 `Theme Pattern Lab — {theme-slug} {version}`：

1. 记录 MCP 失败类型：无工具、无权限、接口错误、只返回名称或缺少原始 content。
2. 在浏览器中只查看当前页面必需 Pattern 的真实预览、区块树和可编辑字段，不遍历全部 Pattern。
3. 只把当前页面会使用的 Pattern 插入 Pattern Lab；无需把无关模块写入页面。
4. 保存草稿，不发布。
...
- 草稿阶段记录后台代表性复杂模块、公共同步模块和 1440、390 前台结果。
- 发布前或出现断点风险时，再记录全部主要模块与 1440、1024、768、390 前台结果。
